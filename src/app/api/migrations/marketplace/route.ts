...
export async function POST() {
  try {
    console.log('[Migration] Starting marketplace tables migration...')
    ...
    `)
    console.log('[Migration] Created marketplace_listings table')
    ...
    `)
    await db.query(`
      CREATE INDEX IF NOT EXISTS idx_mktplace_listings_pending
      ON marketplace_listings(submitted_at DESC)
      WHERE status = 'pending'
    `)
    await db.query(`
      CREATE INDEX IF NOT EXISTS idx_mktplace_listings_approved_active
      ON marketplace_listings(company_id, status)
      WHERE status = 'approved'
    `)
    console.log('[Migration] Created indexes for marketplace_listings')
    ...
    `)
    console.log('[Migration] Created marketplace_listing_history table')
    ...
    `)
    console.log('[Migration] Created marketplace_price_alerts table')
    ...
    `)
    await db.query(`
      CREATE INDEX IF NOT EXISTS idx_mktplace_alerts_active
      ON marketplace_price_alerts(created_at DESC)
      WHERE status = 'active'
    `)
    ...
    `)
    console.log('[Migration] Created marketplace_price_config table')
    ...
    `)
    console.log('[Migration] Created marketplace_listing_stats table')
    ...
    `)
    console.log('[Migration] Created trigger for updated_at')
    ...
    `)
    console.log('[Migration] Created trigger for listing_code generation')
    ...
    `)
    console.log('[Migration] Created trigger for history logging')
    ...
    `)
    console.log('[Migration] Inserted default price config')

    console.log('[Migration] Marketplace tables migration completed successfully!')

    return NextResponse.json({
      success: true,
      message: 'Marketplace tables migration completed successfully',
      tables: [
        'marketplace_listings',
        'marketplace_listing_history',
        'marketplace_price_alerts',
        'marketplace_price_config',
        'marketplace_listing_stats'
      ]
    })
...
export async function GET() {
  try {
    ...
    let stats = {}
    if (existingTables.includes('marketplace_listings')) {
