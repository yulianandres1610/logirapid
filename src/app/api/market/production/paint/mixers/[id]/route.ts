...
export async function DELETE(_: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    await db.query('DELETE FROM paint_mixers WHERE id = $1 AND company_id = $2', [parseInt(id), payload.companyId])
    return NextResponse.json({ success: true, message: 'Mezcladora eliminada' })
  } catch (e: any) {
