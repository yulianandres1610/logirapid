...
export async function POST(request: NextRequest) {
  console.log('[Purchase OCR] Request received')
  ...
  try {
    ...
    console.log('[Purchase OCR] Auth OK, parsing body...')

    let body
    try {
      body = await request.json()
      console.log('[Purchase OCR] Body parsed, fileBase64 length:', body.fileBase64?.length || 0)
    } catch (parseError) {
      console.error('[Purchase OCR] Failed to parse body:', parseError)
      return NextResponse.json({
        success: false,
    ...
    const prompt = `Eres un experto en OCR de facturas de proveedores. Tu tarea es extraer TODOS los productos/items de esta factura.
${contextSection}
EXTRAE LA SIGUIENTE INFORMACIÓN:

1. vendorName: Nombre del proveedor o empresa que emite la factura
...
EJEMPLO DE RESPUESTA CON VARIANTES:
{
  ...
  "items": [
    ...
    { "name": "Jugo Toto Naranja", "quantity": 12, "unitOfMeasure": "unidad", "unitCost": 2.00, "totalCost": 24.00, "sku": null, "barcode": null, "isVariantOf": "Jugo Toto" },
    { "name": "Coca Cola 2L", "quantity": 10, "unitOfMeasure": "unidad", "unitCost": 1.50, "totalCost": 15.00, "sku": "COC-2L", "barcode": null, "isVariantOf": null }
  ],
  "subtotal": 89.00,
  "tax": 7.12,
  "total": 96.12,
...
RESPONDE ÚNICAMENTE CON EL JSON. Sin explicaciones, sin markdown, sin \`\`\`.`
    ...
    console.log('[Purchase OCR] Raw response (full):', text)

    try {
      const extractedData = JSON.parse(text)
      console.log('[Purchase OCR] Parsed data:', JSON.stringify(extractedData, null, 2))

      // Verificar la estructura de la respuesta
      console.log('[Purchase OCR] Response keys:', Object.keys(extractedData))
      console.log('[Purchase OCR] Items type:', typeof extractedData.items, Array.isArray(extractedData.items))
      console.log('[Purchase OCR] Items raw:', extractedData.items)
      ...
      if (!rawItems || !Array.isArray(rawItems) || rawItems.length === 0) {
        ...
        console.log('[Purchase OCR] Using fallback items source:', rawItems.length)
      }
      ...
      if (items.length === 0 && extractedData.total) {
        console.log('[Purchase OCR] No items found, creating generic item from total')
        items.push({
          id: 'item-0',
      ...
      console.log('[Purchase OCR] Final processed items:', items.length, items)
      ...
      console.log('[Purchase OCR] Currency detection:', { detectedCurrency, currencyConfidence, currencyHints })
