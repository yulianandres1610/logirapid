 ...
 * Apply audit count results to adjust stock levels
 *
 * This endpoint:
  ...
  try {
    ...
    try {
      ...
      for (const line of lines) {
        ...
        const systemQty = parseFloat(line.system_quantity) || 0
      ...
      console.log(`[Audit Apply] Successfully applied audit ${count.count_number}: ${adjustmentsApplied} adjustments`)
...
export async function GET(
  ...
  try {
    ...
    // Get lines with differences for preview
    const linesResult = await db.query(`
      SELECT
