...
export default function DoorKioskScannerSelectorPage() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('door-kiosk-scanner-theme')
  ...
  const toggleTheme = () => {
    ...
    localStorage.setItem('door-kiosk-scanner-theme', newTheme)
  }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.redirectTo) {
          // Rewrite redirect from /door-kiosk/ to /door-kiosk-scanner/
          const redirectPath = (result.data.redirectTo as string).replace('/door-kiosk/', '/door-kiosk-scanner/')
  ...
  const selectKiosk = (kiosk: Kiosk) => {
    ...
    router.push(`/door-kiosk-scanner/${kiosk.id}`)
  }
