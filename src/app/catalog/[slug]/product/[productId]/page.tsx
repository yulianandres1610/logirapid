            ...
            {product.wholesaleTiers && product.wholesaleTiers.length > 0 && (
              <div className="rounded-xl border border-blue-200 bg-blue-50 overflow-hidden">
                ...
                <div className="divide-y divide-blue-100">
                  <div className="grid grid-cols-3 px-4 py-2 text-[11px] font-semibold text-blue-600 uppercase">
                    <span>Desde</span>
                    <span className="text-right">Precio CUP</span>
                    <span className="text-right">Precio USD</span>
                  </div>
                  {product.wholesaleTiers.map((tier, i) => (
                    <div key={i} className="grid grid-cols-3 px-4 py-2.5 hover:bg-blue-100/50 transition-colors">
                      <span className="text-sm font-bold text-blue-800">{tier.minQty}+ {product.unit}</span>
                      <span className="text-sm font-bold text-right" style={{ color: primaryColor }}>
                        {tier.priceCUP.toLocaleString('es-ES')} CUP
                        {tier.discountPercent && (
                          <span className="text-[10px] font-normal text-green-600 block">-{tier.discountPercent}%</span>
                        )}
                      </span>
                      <span className="text-sm text-gray-600 text-right">${tier.priceUSD.toFixed(2)}</span>
                    </div>
                  ))}
                </div>
              </div>
