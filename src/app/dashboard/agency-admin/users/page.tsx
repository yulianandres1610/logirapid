...
// Mock data para usuarios de empresa específica
const MOCK_AGENCY_USERS = [
  {
    id: 1,
    firstName: 'Carlos',
    lastName: 'Pérez',
    email: 'carlos.perez@cubaxpress.cu',
    phone: '+53 5 23456789',
    address: 'Calle 25 #789, Playa',
    city: 'La Habana',
    country: 'Cuba',
    role: 'user',
    status: 'active',
    ...
    companies: ['CubaExpress S.A. - Sucursal Central'],
  ...
  },
  {
    ...
    firstName: 'María',
    lastName: 'González',
    email: 'maria.gonzalez@cubaxpress.cu',
    phone: '+53 7 4567890',
    address: 'Avenida 1 #234, Vedado',
    city: 'La Habana',
    country: 'Cuba',
    role: 'user',
    status: 'active',
    ...
    companies: ['CubaExpress S.A. - Sucursal Vedado'],
  ...
  },
  {
    ...
    firstName: 'Luis',
    lastName: 'Martínez',
    email: 'luis.martinez@cubaxpress.cu',
    phone: '+53 5 67890123',
    address: 'Calle 42 #567, Miramar',
    city: 'La Habana',
    country: 'Cuba',
    role: 'admin',
    status: 'active',
    ...
    companies: ['CubaExpress S.A. - Todas las Sucursales'],
  ...
  },
  {
    ...
    firstName: 'Ana',
    lastName: 'López',
    email: 'ana.lopez@cubaxpress.cu',
    phone: '+53 7 89012345',
    address: 'Calle 17 #890, Plaza',
    city: 'La Habana',
    country: 'Cuba',
    role: 'user',
    status: 'inactive',
    ...
    companies: ['CubaExpress S.A. - Sucursal Plaza'],
    transactionsCount: 45
  }
]

// Mock branches de la empresa para asignación
const MOCK_BRANCHES = [
  { id: 1, name: 'CubaExpress S.A. - Sucursal Central', type: 'central' },
  { id: 2, name: 'CubaExpress S.A. - Sucursal Vedado', type: 'branch' },
  { id: 3, name: 'CubaExpress S.A. - Sucursal Miramar', type: 'branch' },
  { id: 4, name: 'CubaExpress S.A. - Sucursal Plaza', type: 'branch' },
  { id: 5, name: 'CubaExpress S.A. - Todas las Sucursales', type: 'all' }
]

// Pasos del formulario de usuario (sin wallet)
const USER_STEPS = [
  { id: 1, title: 'Información Personal', icon: User },
  { id: 2, title: 'Contacto y Ubicación', icon: MapPin },
  { id: 3, title: 'Sucursales', icon: Building2 },
  { id: 4, title: 'Revisión', icon: Check }
]

// Roles de usuario para empresa
const USER_ROLES = [
  { id: 'USER', name: 'Vendedor', description: 'Solo puede vender servicios' },
  { id: 'MANAGER', name: 'Manager', description: 'Gestiona usuarios y recarga wallet' },
  { id: 'ADMIN', name: 'Administrador', description: 'Acceso completo a la empresa' }
]
  ...
  const handleCreateUser = async () => {
    try {
      const response = await fetch('/api/users', {
        ...
        body: JSON.stringify({
          user: {
            ...
            role: formData.role === 'ADMIN' ? 'admin' : formData.role === 'MANAGER' ? 'admin' : 'user',
            password: formData.password,
            isActive: formData.isActive,
            sendEmail: formData.sendEmail
          },
          assignedCompanies: formData.assignedBranches
        }),
      })
      ...
      if (data.success) {
        ...
        alert(`Usuario de empresa creado exitosamente!${data.data.message ? '\n' + data.data.message : ''}`)
      } else {
          ...
          <div className="max-w-6xl mx-auto p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className={cn(
                    ...
                    Completa el formulario para registrar un nuevo usuario en tu empresa
                  </p>
                </div>
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                {/* Paso 3: Asignación de Sucursales */}
                {currentStep === 3 && (
                  <div className={cn(
                    "backdrop-blur-sm border rounded-2xl p-8",
                    theme === 'dark' ? "bg-white/5 border-white/10" : "bg-white/90 border-gray-200"
                    ...
                    {formData.role === 'admin' ? (
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            ...
                            <p className={cn(
                              ...
                              Este rol tiene acceso a todas las sucursales por defecto
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {companies.map((branch: any) => (
                            <button
                              key={branch.id}
                              onClick={() => {
                                const newBranches = formData.assignedBranches.includes(branch.id)
                                  ? formData.assignedBranches.filter((b: number) => b !== branch.id)
                                  : [...formData.assignedBranches, branch.id]
                                setFormData({...formData, assignedBranches: newBranches})
                              }}
                              className={cn(
                                "p-4 rounded-xl border transition-all duration-300 text-left",
                                formData.assignedBranches.includes(branch.id)
                              ...
                              <div className="flex items-center gap-3">
                                <div className={cn(
                                  ...
                                  <Building2 className={cn(
                                    "w-5 h-5",
                                    formData.assignedBranches.includes(branch.id) ? "text-white" : theme === 'dark' ? "text-gray-400" : "text-gray-600"
                                  )} />
                                </div>
                                <div>
                                  <h3 className={cn(
                                  ...
                                  <p className={cn(
                                    ...
                                    {branch.companyType === 'agency' ? 'Agencia' : branch.companyType === 'market' ? 'Mercado' : branch.companyType === 'broker' ? 'Broker' : 'Empresa'}
                                  </p>
                                </div>
                              </div>
                              {formData.assignedBranches.includes(branch.id) && (
                        ...
                        <div className="flex flex-wrap gap-2">
                          {formData.role === 'admin' ? (
                            <span className={cn(
                              ...
                              Todas las sucursales (Administrador)
                            </span>
                          ) : (
                            <>
                              {formData.assignedBranches.map((branchId: number) => {
                                const branch = companies.find(b => b.id === branchId)
                                return branch ? (
                                  <span
                                    key={branchId}
                                    className={cn(
              ...
              <div className="flex items-center justify-center h-64">
                <div className="text-center">
                  ...
                  <p className={cn(
                    ...
                    Cargando usuarios de la empresa...
                  </p>
                </div>
              </div>
            ) : (
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className={cn(
                    ...
                    Gestiona los usuarios de tu empresa y sus sucursales asignadas
                  </p>
                </div>
            ...
            >
              <div className="flex flex-col sm:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <input
                    type="text"
                    placeholder="Buscar usuarios de la empresa..."
                    value={searchTerm}
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1">
                    {user.companies.slice(0, 2).map((company: any, idx: number) => (
                      ...
                      >
                        {company.replace('CubaExpress S.A. - ', '')}
                      </span>
                    ))}
