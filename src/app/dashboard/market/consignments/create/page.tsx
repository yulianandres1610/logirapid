...
export default function CreateConsignmentOrderPage() {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        ...
        img.onload = () => {
          ...
          console.log('[Consignment AI] Image compressed:', {
  ...
  // Upload real image and clean with AI
  const uploadAndCleanImage = useCallback(async (itemId: string, file: File) => {
    ...
    try {
      console.log('[Consignment] Uploading and cleaning image for:', item.name)
      ...
      if (data.success && data.data) {
        ...
        console.error('[Consignment] Image cleaning failed:', data.error)
        setMatchedProducts(prev => prev.map(p =>
          p.id === itemId ? { ...p, isCleaningImage: false } : p
        ))
      }
    } catch (error) {
      console.error('[Consignment] Error cleaning image:', error)
      setMatchedProducts(prev => prev.map(p =>
        p.id === itemId ? { ...p, isCleaningImage: false } : p
      ))
    }
  ...
  const handleSubmitOrder = async () => {
    ...
    try {
      ...
      const newProductLines = orderLines.filter(l => l.isNewProduct && l.productId < 0)
      ...
      for (const [baseName, variants] of variantGroups) {
        ...
        console.log('[Submit Order] Created product with variants:', baseName, 'variants:', variants.length)
      }
      ...
      if (data.success) {
        ...
        if (invoiceFiles.length > 0) {
          try {
            console.log('[Consignment Create] Processing invoices:', invoiceFiles.map(inv => ({
              name: inv.name,
              hasFile: !!inv.file,
            ...
            console.log('[Consignment Create] Local files:', localFileInvoices.length, 'Phone uploads:', phoneUploadedInvoices.length)
            ...
            if (localFileInvoices.length > 0) {
              ...
              if (filesAdded > 0) {
                ...
                console.log('[Consignment Create] Local upload result:', uploadResult)
              }
            }
            ...
            if (phoneUploadedInvoices.length > 0) {
              ...
              if (existingFiles.length > 0) {
                ...
                console.log('[Consignment Create] Phone upload register result:', registerResult)
              }
            }
          } catch (uploadError) {
  ...
  const printWithSilentService = async () => {
    if (!createdOrder || !selectedSupplier || !selectedWarehouse) return
                            ...
                            <div key={item.id} className={cn('p-4 rounded-xl border', item.action === 'ignore' ? 'opacity-50' : '', theme === 'dark' ? 'bg-blue-900/20 border-blue-800/50' : 'bg-blue-50 border-blue-200')}>
                              <div className="flex items-start gap-3">
                                ...
                                <div className="relative flex-shrink-0">
                                  ...
                                  {/* Botones para imagen: subir y limpiar O generar */}
                                  <div className="absolute -bottom-1 -right-1 flex gap-1">
                                    ...
                                    />
                                    {/* Botón para subir imagen real y limpiar con IA */}
                                    <button
                                      onClick={() => document.getElementById(`upload-image-${item.id}`)?.click()}
                                      disabled={item.isCleaningImage || item.isGeneratingImage || item.action === 'ignore'}
                                      className={cn(
                                        'p-1.5 rounded-full shadow-lg transition-all',
                                        item.isCleaningImage
                                          ? 'bg-blue-500 cursor-wait'
                                          : 'bg-blue-500 hover:bg-blue-600',
                                        (item.action === 'ignore' || item.isGeneratingImage) && 'opacity-50 cursor-not-allowed'
                                      )}
                                    ...
                                    {/* Botón para generar imagen desde cero con IA */}
                                    <button
                                      onClick={() => generateImageWithAI(item.id)}
                                      disabled={item.isGeneratingImage || item.isCleaningImage || item.action === 'ignore'}
                                      className={cn(
                                        'p-1.5 rounded-full shadow-lg transition-all',
                                        ...
                                        (item.action === 'ignore' || item.isCleaningImage) && 'opacity-50 cursor-not-allowed'
                                      )}
