...
export default function EditProductPage() {
  ...
  // Cargar el producto existente y sus variantes
  useEffect(() => {
          ...
          if (data.success && data.data) {
            ...
            if (p.variants && p.variants.length > 0) {
              const loadedVariants: Variant[] = p.variants.map((v: any) => ({
                id: v.id?.toString() || `var-${Date.now()}-${Math.random()}`,
                name: v.name || '',
                sku: v.sku || '',
                barcode: v.barcode || '',
              ...
              // Inferir tipos de variantes desde las opciones
              const typesMap = new Map<string, Set<string>>()
              loadedVariants.forEach(v => {
                v.options.forEach(opt => {
                  if (!typesMap.has(opt.type)) {
                    typesMap.set(opt.type, new Set())
                  }
                  typesMap.get(opt.type)!.add(opt.value)
                })
              })
              const inferredTypes: VariantType[] = []
              typesMap.forEach((values, name) => {
                inferredTypes.push({
  ...
  // Check for existing images by barcode (soporta múltiples) - solo si no hay imagen cargada
  useEffect(() => {
    if (!productLoaded) return // No buscar mientras carga el producto
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If we have a base64 data URL, upload it to storage first
        ...
        console.log('[Product Edit] Uploading base64 image to storage...')
        try {
          ...
          if (uploadData.success && uploadData.data?.imageUrl) {
            ...
            console.log('[Product Edit] Base64 uploaded:', imageUrl)
          }
        } catch (uploadError) {
          console.error('[Product Edit] Failed to upload base64:', uploadError)
        }
      }
      ...
      console.log('[Product Edit] Using image URL:', imageUrl)
      ...
      const response = await fetch(`/api/market/products/${productId}`, {
        ...
        body: JSON.stringify({
          ...
          variants: formData.hasVariants ? variants.map(v => ({
            id: v.id && !v.id.startsWith('var-') ? parseInt(v.id) : undefined, // Solo enviar ID si existe en DB
            name: v.name,
            sku: v.sku,
            barcode: v.barcode,
  ...
  // Pantalla de carga mientras se carga el producto
  if (loading) {
    return (
      <ProtectedRoute>
        <DashboardLayout>
      ...
      <div className="max-w-4xl xl:max-w-5xl mx-auto space-y-6 sm:space-y-8 relative">
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <motion.button
                  ...
                  className="flex items-center gap-2 px-3 py-2 rounded-xl text-sm bg-red-500 text-white hover:bg-red-600 transition-all"
                >
            ...
            <motion.button
              ...
              className={cn(
                ...
                "transition-colors duration-200 hidden", // Oculto porque ya tenemos header
                theme === 'dark'
            ...
            <div className="flex justify-between items-center gap-4">
              ...
              {currentStep === 'review' ? (
                ...
                >
                  {(saving || uploadingImage) && <Loader2 className="w-5 h-5 animate-spin" />}
