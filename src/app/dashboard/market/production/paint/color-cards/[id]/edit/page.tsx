...
import { Plus, Loader2, X, Check, Droplets, Trash2, ArrowLeft, ArrowRight, FlaskConical, CheckCircle2 } from 'lucide-react'
...
const STEPS: { id: Step; label: string; icon: any }[] = [
  { id: 'info', label: 'Información', icon: FlaskConical },
  { id: 'base', label: 'Base', icon: FlaskConical },
  { id: 'tints', label: 'Tintas', icon: Droplets },
  { id: 'review', label: 'Confirmar', icon: CheckCircle2 },
]

export default function EditColorCardPage({ params }: { params: Promise<{ id: string }> }) {
  const { theme } = useTheme()
  const isDark = theme === 'dark'
  const router = useRouter()
  const resolvedParams = use(params)
  const colorId = resolvedParams.id
  ...
  const [tints, setTints] = useState<TintLine[]>([])

  useEffect(() => {
    Promise.all([
      fetch(`/api/market/production/paint/color-cards/${colorId}`).then(r => r.json()),
      fetch('/api/market/production/paint/base-types').then(r => r.json()),
      fetch('/api/market/products?limit=500').then(r => r.json())
    ]).then(([ccData, btData, pData]) => {
      if (ccData.success) {
        const cc = ccData.data
        setForm({ code: cc.code, name: cc.name, baseTypeId: String(cc.baseTypeId), hexColor: cc.hexColor || '#FFFFFF', notes: cc.notes || '' })
        setTints(cc.tints.length > 0
          ? cc.tints.map((t: any, i: number) => ({ id: String(i), tintProductId: t.tintProductId, quantityPerKg: String(t.quantityPerKg), unit: t.unit || 'g' }))
          : [{ id: '1', tintProductId: null, quantityPerKg: '', unit: 'g' }])
      }
  ...
  const canNext = () => { if (step === 'info') return !!form.name; if (step === 'base') return !!form.baseTypeId; return true }
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      const res = await fetch(`/api/market/production/paint/color-cards/${colorId}`, {
        method: 'PUT', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: form.name, baseTypeId: parseInt(form.baseTypeId), hexColor: form.hexColor, notes: form.notes || null,
    ...
    <ProtectedRoute><DashboardLayout>
      <div className="max-w-3xl mx-auto px-4 pt-10 pb-20 relative">
        ...
        <div className="text-center mb-8">
          <h1 className={cn('text-2xl font-bold', isDark ? 'text-white' : 'text-gray-900')}>Editar Color — {form.code}</h1>
          <p className="text-sm text-gray-500 mt-1">Modifica la información y tintas</p>
        </div>
        ...
        <div className="flex items-center justify-center gap-2 mb-10">
          {STEPS.map((s, i) => { const active = s.id === step; const done = i < si; const Icon = s.icon; return (<div key={s.id} className="flex items-center">
            <div className="flex flex-col items-center"><motion.div animate={{ scale: active ? 1.1 : 1 }} className={cn('w-10 h-10 rounded-full flex items-center justify-center relative', done ? 'bg-green-500 text-white' : active ? 'bg-orange-500 text-white' : isDark ? 'bg-gray-700 text-gray-400' : 'bg-gray-200 text-gray-400')}>
              {done ? <Check className="w-5 h-5" /> : <Icon className="w-5 h-5" />}{active && <motion.div animate={{ scale: [1, 1.3, 1], opacity: [0.5, 0, 0.5] }} transition={{ duration: 2, repeat: Infinity }} className="absolute inset-0 rounded-full bg-orange-500" />}
            </motion.div><span className={cn('text-xs mt-1.5 font-medium', active ? 'text-orange-500' : done ? 'text-green-500' : 'text-gray-400')}>{s.label}</span></div>
        ...
        <AnimatePresence mode="wait">
          {step === 'info' && <motion.div key="info" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
            ...
            <div><label className={lc}>Notas</label><textarea value={form.notes} onChange={e => setForm({ ...form, notes: e.target.value })} rows={2} className={ic} /></div>
          </motion.div>}

          {step === 'base' && <motion.div key="base" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
            <p className={cn('text-center font-bold text-lg mb-6', isDark ? 'text-white' : 'text-gray-900')}>Tipo de Base</p>
            {baseTypes.map(bt => (
              <motion.button key={bt.id} whileTap={{ scale: 0.99 }} onClick={() => setForm({ ...form, baseTypeId: String(bt.id) })}
                className={cn('w-full p-5 rounded-2xl border text-left flex items-center gap-4',
                  form.baseTypeId === String(bt.id) ? 'border-orange-500 bg-orange-50 dark:bg-orange-900/20 shadow-lg shadow-orange-500/10' : isDark ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white')}>
                <FlaskConical className={cn('w-7 h-7', form.baseTypeId === String(bt.id) ? 'text-orange-600' : 'text-gray-400')} />
          ...
          {step === 'tints' && <motion.div key="tints" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            <p className="text-center text-sm text-gray-500 mb-6">Cantidad por kg de base</p>
              ...
              <div key={t.id} className={cn('p-4 rounded-2xl border', isDark ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-gray-50')}>
                ...
                <div className="grid grid-cols-[1fr_100px_70px] gap-3">
                  <select value={t.tintProductId || ''} onChange={e => setTints(tints.map(x => x.id === t.id ? { ...x, tintProductId: parseInt(e.target.value) || null } : x))}
                    className={cn('px-3 py-2.5 rounded-xl border text-sm', isDark ? 'bg-gray-700 border-gray-600 text-white' : 'bg-white border-gray-200')}><option value="">Seleccionar...</option>{products.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>
                  <input type="number" step="any" placeholder="Cant." value={t.quantityPerKg} onChange={e => setTints(tints.map(x => x.id === t.id ? { ...x, quantityPerKg: e.target.value } : x))}
            ...
            <button onClick={() => setTints([...tints, { id: Date.now().toString(), tintProductId: null, quantityPerKg: '', unit: 'g' }])}
              ...
              <Plus className="w-4 h-4" /> Agregar tinta
            </button>
          </motion.div>}
          ...
          {step === 'review' && <motion.div key="review" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-md mx-auto text-center">
            <div className={cn('rounded-2xl border p-8', isDark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-white')}>
              ...
              <div className={cn('p-4 rounded-xl text-left', isDark ? 'bg-gray-900' : 'bg-gray-50')}>
                ...
                <p className="text-sm flex items-center gap-2"><Droplets className="w-4 h-4 text-purple-500" /> <strong>{validTints.length} tintas</strong></p>
        ...
        <div className="flex items-center justify-between mt-10">
          ...
          {step === 'review'
            ? <button onClick={handleSave} disabled={saving} className="flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl font-medium text-sm disabled:opacity-50">
                {saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} {saving ? 'Guardando...' : 'Guardar Cambios'}
              </button>
