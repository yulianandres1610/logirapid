...
export default function CreateBaseTypePage() {
  ...
  useEffect(() => {
    fetch('/api/market/production/formulas?limit=100&isActive=true').then(r => r.json()).then(d => {
      if (d.success) setFormulas((d.data?.formulas || d.data || []).map((f: any) => ({ id: f.id, code: f.code, name: f.name })))
    }).finally(() => setLoading(false))
  }, [])

  const si = STEPS.findIndex(s => s.id === step)
  const canNext = () => { if (step === 'info') return !!form.code && !!form.name; return true }
  ...
  return (
    <ProtectedRoute><DashboardLayout>
      <div className="max-w-3xl mx-auto px-4 pt-10 pb-20 relative">
        ...
        <div className="text-center mb-8"><h1 className={cn('text-2xl font-bold', isDark ? 'text-white' : 'text-gray-900')}>Nuevo Tipo de Base</h1><p className="text-sm text-gray-500 mt-1">Configura la base y enlaza con una fórmula</p></div>
        ...
        <AnimatePresence mode="wait">
          {step === 'info' && <motion.div key="info" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5 max-w-lg mx-auto">
            <div className="grid grid-cols-2 gap-4"><div><label className={lc}>Código *</label><input value={form.code} onChange={e => setForm({ ...form, code: e.target.value.toUpperCase() })} placeholder="BASE_CLARA" className={ic} /></div><div><label className={lc}>Nombre *</label><input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Base Clara" className={ic} /></div></div>
            <div><label className={lc}>Descripción</label><textarea value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} rows={3} placeholder="Descripción del tipo de base..." className={ic} /></div>
          </motion.div>}
          ...
          {step === 'formula' && <motion.div key="formula" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-lg mx-auto space-y-4">
            <p className={cn('text-center font-bold text-lg mb-6', isDark ? 'text-white' : 'text-gray-900')}>Enlazar con Fórmula de Producción</p>
            {formulas.map(f => (
            ...
            {formulas.length === 0 && <p className="text-center text-gray-400 py-8">No hay fórmulas activas</p>}
            <button onClick={() => setForm({ ...form, formulaId: '' })} className="text-xs text-gray-400 hover:text-gray-500 block mx-auto">Continuar sin fórmula</button>
            <div className="pt-4"><label className={lc}>Rendimiento por tanda (kg)</label><input type="number" step="any" value={form.yieldKg} onChange={e => setForm({ ...form, yieldKg: e.target.value })} className={ic} /><p className="text-xs text-gray-400 mt-1">Cuántos kg de mezcla produce una tanda de la fórmula</p></div>
          </motion.div>}
          ...
          {step === 'review' && <motion.div key="review" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="max-w-md mx-auto text-center">
            <div className={cn('rounded-2xl border p-8', isDark ? 'border-gray-700 bg-gray-800/50' : 'border-gray-200 bg-white')}>
              ...
              <div className={cn('p-4 rounded-xl text-left', isDark ? 'bg-gray-900' : 'bg-gray-50')}>
                {selectedFormula ? <p className="text-sm mb-1"><strong>Fórmula:</strong> {selectedFormula.code} — {selectedFormula.name}</p> : <p className="text-sm text-gray-400 mb-1">Sin fórmula enlazada</p>}
        ...
        <div className="flex items-center justify-between mt-10">
          ...
          {step === 'review'
            ? <button onClick={handleCreate} disabled={saving} className="flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-xl font-medium text-sm disabled:opacity-50">{saving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Check className="w-4 h-4" />} {saving ? 'Guardando...' : 'Crear Tipo de Base'}</button>
