...
function MetricCard({
  ...
  return (
    <motion.div
      ...
      className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${colors[color]} p-4 md:p-5 text-white shadow-lg ${onClick ? 'cursor-pointer active:opacity-90 transition-opacity touch-manipulation' : ''}`}
    >
      ...
      <div className="relative">
        <div className="flex items-start justify-between mb-2 md:mb-3">
          ...
          {trend && trendValue && (
            <div className={`hidden sm:flex items-center gap-1 text-[10px] md:text-xs px-2 py-1 rounded-full ${
        ...
        <p className="text-2xl md:text-3xl font-bold mb-0.5 md:mb-1">
          {typeof value === 'number' ? <AnimatedNumber value={value} /> : value}
        </p>
        <p className="text-xs md:text-sm font-medium text-white/90">{title}</p>
        {subtitle && <p className="text-[10px] md:text-xs text-white/70 mt-0.5 truncate">{subtitle}</p>}
      </div>
    </motion.div>
  )
}
...
function AlertCard({
  ...
  return (
    ...
    >
      <Link href={href}>
        <div className={`flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-xl border-2 ${colors[color]} active:opacity-80 transition-all cursor-pointer group touch-manipulation`}>
          <div className={`p-2 md:p-3 rounded-xl ${iconColors[color]}`}>
            <Icon className="w-4 h-4 md:w-5 md:h-5" />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5 md:gap-2">
              <span className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">{count}</span>
              <span className="text-sm md:text-base font-semibold text-gray-900 dark:text-white">{title}</span>
            </div>
...
function BalanceCard({
  ...
  return (
    <motion.div
      ...
      transition={{ delay }}
      className="bg-white dark:bg-[#1e1e2f] rounded-xl p-3 md:p-4 border border-gray-200 dark:border-gray-700/50"
    >
      <div className="flex items-center justify-between mb-2 md:mb-3">
        <div className="flex items-center gap-1.5 md:gap-2">
          <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center text-white text-[10px] md:text-xs font-bold">
            {currency}
          </div>
          <span className="font-medium text-sm md:text-base text-gray-700 dark:text-gray-300">{currency}</span>
        </div>
        {reserved > 0 && (
          <span className="text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded-full bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400">
            {symbol}{reserved.toLocaleString()}
          </span>
        )}
      </div>
      ...
      <p className="text-[10px] md:text-xs text-gray-500 dark:text-gray-400 mt-0.5 md:mt-1">Disponible</p>
    </motion.div>
  )
}
...
function QuickAction({
  ...
  return (
    ...
    >
      <Link href={href}>
        <div className={`relative group p-4 md:p-5 rounded-2xl bg-gradient-to-br ${color} text-white active:opacity-90 transition-all cursor-pointer shadow-lg touch-manipulation`}>
          {badge !== undefined && badge > 0 && (
            <div className="absolute -top-2 -right-2 w-5 h-5 md:w-6 md:h-6 rounded-full bg-red-500 text-white text-[10px] md:text-xs font-bold flex items-center justify-center shadow-lg">
              {badge > 99 ? '99+' : badge}
            </div>
          )}
          <div className="flex items-center gap-3 md:gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs md:text-sm text-white/80 truncate">{description}</p>
            </div>
            <ArrowRight className="w-4 h-4 md:w-5 md:h-5 opacity-70 group-hover:translate-x-1 transition-transform flex-shrink-0" />
          </div>
        </div>
      </Link>
    </motion.div>
        ...
        <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
          ...
          >
            <div>
              ...
              <div className="text-gray-500 dark:text-gray-400 flex flex-wrap items-center gap-x-2 gap-y-1 mt-1 text-xs md:text-sm">
                <div className="flex items-center gap-1">
                  <MapPin className="w-3.5 h-3.5 md:w-4 md:h-4" />
                  <span>{walletData?.municipality}, {walletData?.province}</span>
                </div>
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-700 active:bg-gray-100 dark:active:bg-gray-600 transition-colors shadow-sm min-h-[44px] touch-manipulation self-start sm:self-auto"
            >
          ...
          {(stats?.pending || 0) + (stats?.confirmed || 0) + (stats?.inDelivery || 0) > 0 && (
            <motion.div
              ...
              className="bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 rounded-2xl p-3 md:p-4 border border-amber-200 dark:border-amber-800"
            >
              <div className="flex items-center gap-2 mb-2 md:mb-3">
                <Bell className="w-4 h-4 md:w-5 md:h-5 text-amber-600" />
                <h3 className="font-bold text-sm md:text-base text-gray-900 dark:text-white">Acciones Pendientes</h3>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            <motion.div
              ...
              className="lg:col-span-2 bg-white dark:bg-[#1e1e2f] rounded-2xl border border-gray-200 dark:border-gray-700/50 p-4 md:p-5 shadow-sm"
            >
              <div className="flex items-start justify-between mb-3 md:mb-4">
                <div>
                  ...
                  <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Entregas últimos 7 días</p>
                </div>
                <div className="text-right">
                  <p className="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">
                    <AnimatedNumber value={stats?.delivered || 0} />
                  </p>
                  <p className="text-[10px] md:text-xs text-gray-500">total entregas</p>
                </div>
              </div>
            ...
            <motion.div
              ...
              className="bg-white dark:bg-[#1e1e2f] rounded-2xl border border-gray-200 dark:border-gray-700/50 p-4 md:p-5 shadow-sm"
            >
              <div className="flex items-center justify-between mb-3 md:mb-4">
                ...
                <Link href="/dashboard/broker/orders" className="text-xs md:text-sm text-blue-500 hover:text-blue-600 flex items-center gap-1 touch-manipulation">
          ...
          >
            <div className="flex items-start justify-between mb-3 md:mb-4 gap-2">
              <div>
                <h3 className="font-bold text-base md:text-lg text-gray-900 dark:text-white flex items-center gap-2">
                  <CircleDollarSign className="w-4 h-4 md:w-5 md:h-5 text-green-500" />
                  Saldos por Moneda
                </h3>
                <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400">Fondos disponibles para entregas</p>
              </div>
              <Link href="/dashboard/broker/wallet" className="text-xs md:text-sm text-blue-500 hover:text-blue-600 flex items-center gap-1 touch-manipulation flex-shrink-0">
          ...
          >
            {[
              ...
              <motion.div
                ...
                className={`p-2 md:p-3 rounded-xl ${stat.color} text-center`}
              >
