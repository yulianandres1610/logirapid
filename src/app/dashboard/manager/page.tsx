...
export default function ManagerDashboard() {
  ...
  // Estadísticas específicas para Manager
  const stats = [
    {
  ...
  const recentActivity = [
    { id: 1, user: "Nuevo usuario creado", action: "Ana Martínez", time: "hace 5 min", type: "success" },
    { id: 2, user: "Wallet recargado", action: "$500 agregados", time: "hace 12 min", type: "success" },
    { id: 3, user: "Transacción", action: "Remesa procesada", time: "hace 25 min", type: "info" },
    { id: 4, user: "Documento subido", action: "Contrato actualizado", time: "hace 1 hora", type: "warning" }
  ]

  const quickActions = [
    { title: "Nuevo Usuario", icon: Users, color: "bg-blue-500", href: "/dashboard/manager/users" },
    { title: "Recargar Wallet", icon: Wallet, color: "bg-green-500", href: "/dashboard/manager/wallet" },
    { title: "Transacciones", icon: CreditCard, color: "bg-purple-500", href: "/dashboard/manager/transactions" },
    { title: "Documentos", icon: FileText, color: "bg-orange-500", href: "/dashboard/manager/documents" }
  ]
    ...
    <ProtectedRoute requiredRole="MANAGER">
      <DashboardLayout>
        <div className="max-w-7xl mx-auto space-y-8">
          {/* Welcome Header - Manager Specific */}
          <motion.div
          ...
          >
            <h1 className={cn(
              ...
              Bienvenido, {user?.name?.split(' ')[0] || 'Manager'}
            </h1>
            <p className={cn(
              "text-xs",
              theme === 'dark' ? "text-gray-400" : "text-gray-600"
            )}>
              Panel de gestión - Tu Empresa
            </p>
          </motion.div>

          {/* Stats Grid - Manager Cards */}
          ...
          {/* Activity Section - Manager Focus */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            >
              <h2 className={cn(
                "text-xl font-light mb-6",
                theme === 'dark' ? "text-white" : "text-gray-900"
              )}>Métricas de Gestión</h2>

              <div className="space-y-6">
                {[
                  { name: "Usuarios Activos", percentage: 85, color: theme === 'dark' ? "bg-exa-secondary" : "bg-exa-primary" },
                  { name: "Transacciones Procesadas", percentage: 92, color: theme === 'dark' ? "bg-exa-primary" : "bg-exa-secondary" },
                  { name: "Documentos Completos", percentage: 78, color: "bg-green-500" },
                  { name: "Solicitudes Pendientes", percentage: 23, color: "bg-yellow-500" }
                ].map((metric, index) => (
                  <motion.div
                    key={metric.name}
