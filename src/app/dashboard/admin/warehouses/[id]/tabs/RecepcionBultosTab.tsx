...
export default function RecepcionBultosTab({ warehouse, onRefresh }: RecepcionBultosTabProps) {
  ...
  const handleAddCode = async (code: string) => {
    ...
    if (bulto) {
      // Validar estado: permitir recibir cualquier estado excepto 'disponible' y 'entregado'
      let status: 'valid' | 'invalid' | 'already-here' = 'invalid'
      let errorMessage = ''

      // Ya está en este almacén
      if (bulto.warehouse_id === warehouse.id && bulto.estado === 'en_almacen') {
        status = 'already-here'
      }
      // No se puede recibir si está disponible o ya fue entregado
      else if (bulto.estado === 'disponible' || bulto.estado === 'entregado') {
        status = 'invalid'
        errorMessage = `Estado inválido: ${bulto.estado}. No se puede recibir empaques ${bulto.estado}.`
      }
      ...
      // Válido para recibir (asignado, recogida, en_transito, etc.)
      else {
        status = 'valid'
      }
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.empaques && data.empaques.length > 0) {
            ...
            // Ya está en este almacén
            if (empaque.warehouse_id === warehouse.id && empaque.estado === 'en_almacen') {
              status = 'already-here'
            }
            // No se puede recibir si está disponible o ya fue entregado
            else if (empaque.estado === 'disponible' || empaque.estado === 'entregado') {
              status = 'invalid'
              errorMessage = `Estado inválido: ${empaque.estado}. No se puede recibir empaques ${empaque.estado}.`
            }
            // Debe tener orden asignada
  ...
  const handleReceiveAll = async () => {
    const bultosToTransfer = scannedBultos.filter(
      b => b.status === 'valid' && b.data
    )

    if (bultosToTransfer.length === 0) {
      showNotification('warning', 'Sin bultos para transferir', 'No hay bultos válidos para recibir')
      return
    }
    ...
    try {
      ...
      for (const bulto of bultosToTransfer) {
        try {
          const response = await fetch(`/api/empaques/${bulto.data.id}`, {
            ...
            body: JSON.stringify({
              ...
              usuarioId: 1,
              usuarioNombre: 'Admin',
              accion: 'recepcion_almacen',
              ubicacion: warehouse.name,
              notas: `Recibido en ${warehouse.name} - Recepción Bultos`
            })
          })
          ...
          if (response.ok) {
            ...
            setScannedBultos(prev =>
              prev.map(b =>
                b.code === bulto.code
                  ? { ...b, status: 'already-here', data: { ...b.data, warehouse_id: warehouse.id, warehouse_name: warehouse.name } }
                  : b
              )
            )
          } else {
      ...
      if (successCount > 0) {
        showNotification(
          ...
          `${successCount} bulto${successCount !== 1 ? 's' : ''} recibido${successCount !== 1 ? 's' : ''} ${errorCount > 0 ? `(${errorCount} error(es))` : ''}`
        )
      ...
      if (errorCount > 0 && successCount === 0) {
        showNotification('error', 'Error', 'No se pudieron transferir los bultos')
      }
    } catch (error) {
      console.error('Error receiving bultos:', error)
  ...
  const stats = {
    ...
    alreadyHere: scannedBultos.filter(b => b.status === 'already-here').length,
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Escanea múltiples códigos para recibir bultos en {warehouse.name}
          </p>
        </div>
      </div>
                    ...
                    <div className="flex-1">
                      ...
                      {bulto.data && (
                        <div className="text-xs text-gray-600 dark:text-gray-400 space-y-1 mt-2">
                          ...
                          <p className="mt-1">Origen: {bulto.data.warehouse_name}</p>
              ...
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Recibir {stats.valid} Bulto{stats.valid !== 1 ? 's' : ''}
                  </>
                )}
              </button>
            </div>
      ...
      {stats.total === 0 && (
        <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-700">
          ...
          <p className="text-gray-600 dark:text-gray-400 max-w-md mx-auto">
            Usa el campo de arriba para comenzar a escanear los códigos de los bultos que quieres recibir en este almacén
          </p>
        </div>
      )}
    </div>
