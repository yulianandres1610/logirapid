...
export async function generatePurchaseInvoicePdf(data: PurchaseInvoiceData): Promise<Buffer> {
  ...
  if (supplierCode) {
    drawText(`Codigo: ${supplierCode}`, 50, y, { size: 9 })
    y -= 13
  }
  ...
  if (data.warehouseName) {
    drawText(`Almacen: ${data.warehouseName}`, 350, y + 40, { size: 9 })
  }
  if (data.receivedBy) {
  ...
  drawText('Descripcion', 95, tableTop, { font: fontBold, size: 9 })
  ...
  if (data.shipping !== undefined && data.shipping > 0) {
    drawText('Envio:', totalsX, y, { size: 10 })
  ...
  if (data.paymentStatus || data.paymentMethod) {
    ...
    if (data.paymentMethod) {
      drawText(`Metodo de pago: ${data.paymentMethod}`, 50, y - 5, { size: 9 })
      y -= 15
    }
