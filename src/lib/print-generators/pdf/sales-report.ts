...
export async function generateSalesReportPdf(data: SalesReportData): Promise<Buffer> {
  ...
  if (data.warehouseName) {
    drawText(`Almacen: ${data.warehouseName}`, 50, y, { size: 10, color: gray })
    y -= 15
  }

  // Date info on the right
  if (data.periodStart && data.periodEnd) {
    drawText(`Periodo: ${data.periodStart} - ${data.periodEnd}`, 380, 725, { size: 10 })
  } else {
  ...
  if (data.salesByPayment && data.salesByPayment.length > 0) {
    ...
    drawText('VENTAS POR METODO DE PAGO', 55, y, { font: fontBold, size: 11, color: darkBlue })
    y -= 25
  ...
  if (data.topProducts && data.topProducts.length > 0) {
    ...
    drawText('PRODUCTOS MAS VENDIDOS', 55, y, { font: fontBold, size: 11, color: darkBlue })
    y -= 25
