...
export async function generateCashRegisterReportPdf(data: CashRegisterReportData): Promise<Buffer> {
  ...
  if (data.sessionNumber) {
    drawText(`Sesion: ${data.sessionNumber}`, 50, y, { size: 10, color: gray })
    y -= 15
  }
  ...
  if (data.openedBy) {
    drawText(`Abrio: ${data.openedBy}`, 380, 690, { size: 9, color: gray })
  }
  if (data.closedBy) {
    drawText(`Cerro: ${data.closedBy}`, 380, 675, { size: 9, color: gray })
  }
