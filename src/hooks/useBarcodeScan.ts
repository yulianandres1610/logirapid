...
export function useBarcodeScan(options: UseBarcodeScanOptions) {
  ...
  const processBarcode = useCallback((barcode: string, event?: KeyboardEvent) => {
    ...
    if (trimmed.length >= minLength) {
      ...
      console.log('[Barcode Scanner] Detected:', JSON.stringify(trimmed), 'length:', trimmed.length)
      onScan(trimmed)
    } else {
      console.log('[Barcode Scanner] Rejected (too short):', JSON.stringify(barcode))
    }
    resetBuffer()
