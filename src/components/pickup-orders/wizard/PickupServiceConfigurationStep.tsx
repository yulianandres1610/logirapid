...
export default function ServiceConfigurationStep({ wizardData, updateWizardData, setCanProceed }: Props) {
  ...
  const validateConfig = (config: ServiceConfig) => {
    ...
    return config.boxes.every(box => {
      // PICKUP ORDERS: Box code is OPTIONAL
      // Only validate if a code is provided
      if (config.requiresBoxCode && box.boxCode && box.boxCode.trim() !== '') {
        // Si proporciona código, debe estar validado como 'valid'
        if (box.validationStatus !== 'valid') return false
      }

      // PICKUP ORDERS: Weight is OPTIONAL
      // Only validate if a weight is provided
      if (config.requiresWeight && box.weight && box.weight.trim() !== '') {
        const weightValue = parseFloat(box.weight)
        if (isNaN(weightValue) || weightValue <= 0) return false
      }

      // PICKUP ORDERS: Content Type is REQUIRED (único campo obligatorio)
