...
import { CheckCircle, Printer, FileText, Loader2, PartyPopper, MessageSquare } from 'lucide-react'
...
export default function PickupOrderConfirmationStep({ wizardData, updateWizardData, setCanProceed }: Props) {
  ...
  const sendWhatsAppNotification = async () => {
    ...
    try {
      ...
      console.log('[WhatsApp] Reenviando WhatsApp para orden ID:', orderId)
      ...
      if (data.success) {
        console.log('[WhatsApp] WhatsApp reenviado exitosamente. SID:', data.data?.messageId)
        showNotification(
          'success',
          'WhatsApp Enviado',
          `Mensaje enviado exitosamente a ${data.data?.to || wizardData.sender.phone}`
