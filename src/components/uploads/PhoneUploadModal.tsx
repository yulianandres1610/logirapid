...
interface PhoneUploadModalProps {
  isOpen: boolean
  onClose: () => void
  purpose: 'purchase_invoice' | 'expense_receipt' | 'product_image' | 'employee_contract_photo' | 'fixed_asset_image'
  referenceType?: 'purchase' | 'expense' | 'consignment' | 'contract'
  referenceId?: number
  onUploadComplete: (fileUrl: string, fileName: string, fileSize?: number, fileType?: string, signedUrl?: string) => void
}
      ...
      try {
        ...
        if (data.success) {
          if (data.data.status === 'uploaded') {
            ...
            onUploadComplete(data.data.fileUrl, data.data.fileName, data.data.fileSize, data.data.fileType, data.data.signedUrl)
