...
async function performUpdate(config) {
  ...
  try {
    ...
    // 1. Download new server.js (try multiple paths)
    console.log("  [Update] Descargando nueva version...");
    let newCode = null;
    const urls = [
      `${serverUrl}/print-service-server.js`,
      `${serverUrl}/print-service/server.js`,
      `${serverUrl}/api/print-agent/install/files?name=server.js`
    ];
    for (const url of urls) {
      if (newCode) break;
      try { const res = await fetch(url); if (res.ok) { const txt = await res.text(); if (txt && txt.includes('VERSION') && !txt.includes('<!DOCTYPE')) newCode = txt; } } catch {}
    }
