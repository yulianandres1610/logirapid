...
export function WholesaleListScreen() {
  const { colors, isDark } = useThemeStore()
  const navigation = useNavigation<NativeStackNavigationProp<OperationsStackParamList>>()
  const { pendingWholesale, fetchPendingWholesale, isLoading } = useOperationsStore()
  const [refreshing, setRefreshing] = React.useState(false)

  useEffect(() => {
    fetchPendingWholesale()
  }, [])

  const onRefresh = async () => {
    setRefreshing(true)
    await fetchPendingWholesale()
    setRefreshing(false)
  }
  ...
  return (
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <View style={{
        ...
        <Text style={{ fontSize: 18, fontWeight: '800', color: colors.text, flex: 1 }}>Despachos</Text>
      </View>
      ...
      >
        {isLoading && pendingWholesale.length === 0 ? (
          ...
          <View style={{ alignItems: 'center', paddingTop: 60 }}>
            <View style={{
              ...
              backgroundColor: isDark ? 'rgba(235,91,12,0.1)' : '#fff7ed', marginBottom: 16,
            }}>
              <Text style={{ fontSize: 36 }}>🚚</Text>
            </View>
            <Text style={{ fontSize: 17, fontWeight: '800', color: colors.text }}>Sin despachos</Text>
            <Text style={{ fontSize: 13, color: colors.textSecondary, marginTop: 4, textAlign: 'center' }}>
              No hay despachos pendientes de completar
            </Text>
          </View>
        ) : (
          <>
            <Text style={{
              ...
              {pendingWholesale.length} despacho{pendingWholesale.length !== 1 ? 's' : ''} pendiente{pendingWholesale.length !== 1 ? 's' : ''}
            </Text>

            {pendingWholesale.map((delivery, i) => (
              <Animated.View key={delivery.id} entering={FadeIn.delay(i * 60).duration(200)}>
                <TouchableOpacity
                  onPress={() => navigation.navigate('WholesaleDetail', { delivery })}
                  activeOpacity={0.7}
                  style={{
                ...
                >
                  <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                    <View style={{
                      ...
                      backgroundColor: isDark ? 'rgba(235,91,12,0.1)' : '#fff7ed', marginRight: 12,
                    }}>
                      <Text style={{ fontSize: 22 }}>🚚</Text>
                    </View>
                    <View style={{ flex: 1 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                        <Text style={{ fontSize: 15, fontWeight: '800', color: colors.text }}>{delivery.invoiceNumber}</Text>
