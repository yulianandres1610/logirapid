...
const APPS = [
  { label: 'Transferir', icon: '🔄', screen: 'TransferCreate', tab: 'Operaciones', color: '#eb5b0c' },
  { label: 'Validar', icon: '✅', screen: 'TransferValidate', tab: 'Operaciones', color: '#10b981' },
  { label: 'Recibir', icon: '📥', screen: 'ReceptionList', tab: 'Operaciones', color: '#3b82f6' },
  { label: 'Despachar', icon: '🚚', screen: 'WholesaleList', tab: 'Operaciones', color: '#8b5cf6' },
  { label: 'Scrap', icon: '♻️', screen: 'ScrapAdjustment', tab: 'Operaciones', color: '#ef4444' },
  { label: 'Movimientos', icon: '📋', screen: 'ProductMovements', tab: 'Escanear', color: '#f59e0b' },
  { label: 'Etiquetas', icon: '🏷️', screen: 'PrintLabel', tab: 'Escanear', color: '#ec4899' },
  { label: 'Config', icon: '⚙️', screen: 'Config', tab: '__tab__', color: '#6b7280' },
]
...
export function DashboardScreen() {
  ...
  const fetchStats = useWarehouseStore((s) => s.fetchStats)
  const { isDark, colors } = useThemeStore()
  const { battery, wifi, time, date } = useDeviceStatus()
  useEffect(() => {
    if (warehouseId) fetchStats()
  }, [warehouseId])
            ...
            <TouchableOpacity
              ...
              onPress={() => {
                if (app.tab === '__tab__') {
                  ...
                  (navigation as any).navigate(app.tab, { screen: app.screen })
                }
              }}
              activeOpacity={0.7}
