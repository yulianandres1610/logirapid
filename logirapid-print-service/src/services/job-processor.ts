    ...
    try {
      if (process.platform === 'win32') {
        ...
        const psScript = [
          ...
          "}",
          "",
          "# Method 4: Use Microsoft Edge (always available on Windows 10/11)",
          "try {",
          "    Write-Host \"Using Microsoft Edge\"",
          "    # First, set the target printer as default temporarily",
          ...
          "        Write-Host \"Set $PrinterName as default\"",
          "    }",
          "    ",
          "    # Use Edge to print",
          ...
          "        # Use Edge with print flag",
          "        $proc = Start-Process -FilePath $edgePath -ArgumentList \"--headless\", \"--disable-gpu\", \"--print-to-pdf-no-header\", \"--run-all-compositor-stages-before-draw\", \"--print-to-printer=$PrinterName\", $PdfPath -PassThru -WindowStyle Hidden",
          ...
          "            $origPrinter = Get-WmiObject -Query \"SELECT * FROM Win32_Printer WHERE Name='$currentDefault'\"",
          "            if ($origPrinter) {",
          ...
          "    }",
          "} catch {",
          "    Write-Host \"Edge method failed: $_\"",
          "}",
          "",
          "# Method 5: Use Windows Print Verb (fallback - may show dialog)",
          "try {",
