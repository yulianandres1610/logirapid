...
export function generateProductLabelZpl(data: ProductLabelData): Buffer {
  ...
  if (data.items && data.items.length > 0) {
    console.log(`[ZPL Generator] Bulk mode: generating ${data.items.length} different labels`)
    ...
    console.log(`[ZPL Generator] Generated ${totalLabels} total labels for ${data.items.length} products`)
